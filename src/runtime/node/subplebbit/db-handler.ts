...
export class DbHandler {
    ...
    async _createOrMigrateTablesIfNeeded() {
        ...
        if (needToMigrate) {
            ...
            if (internalState) {
                ...
                const newChallenges = newSettings.challenges ? await Promise.all(newSettings.challenges.map(getSubplebbitChallengeFromSubplebbitChallengeSettings)) : undefined;
    ...
    private _queryCommentCounts(cid: string): Pick<CommentUpdateType, "replyCount" | "upvoteCount" | "downvoteCount"> {
        const query = `
        SELECT 
        ...
        return this._db.prepare(query).get({ cid, subplebbitAddress: this._subplebbit.address }) as Pick<
            CommentUpdateType,
            "replyCount" | "upvoteCount" | "downvoteCount"
        >;
    }
